...
def read_to_slha(mA, name, output):
	if(name[1] == 't'):
		squark = 1
	elif(name[1] == 'b'):
		squark = 2
	...
	effb = (1. + (1. + factor)*deltab)/(1 + deltab)
...
Block IBPPARA
	1	0.0001	#lambda
	2	0.000000001	#epsilon
"""
	dico = {'alphas':alphas, 'mu':mu, 'Aqtree':Aqtree, 'beta':np.arctan(tanb), 'sintheta':sintheta, 'costheta':costheta, 'totalcorr':totalcorrfactor, 'gq12':gq12, 'squark':squark, 'mA':float(mA), 'mQ':mQ, 'mQQQ':mQQQ, 'mG':mG, 'msq1':msq1, 'msq2':msq2}
	return open(output, 'w').write(text%dico)

read_to_slha(mA, name, output)
