...
#https://github.com/JohannesBuchner/PyMultiNest/blob/master/pycuba/__init__.py
...
integrand_type = ctypes.CFUNCTYPE(c_int, POINTER(c_int),  POINTER(c_double), POINTER(c_int), POINTER(c_double), c_void_p)
peakfinder_type = ctypes.CFUNCTYPE(c_void_p, POINTER(c_int), POINTER(BOUNDS), POINTER(c_int), POINTER(c_double))
	...
	*statefile*: a filename for storing the internal state.
	...
	return dict(neval=neval.value, fail=fail.value, comp=comp.value,
		results=[{
			...
			'error':error[comp],
			'prob':prob[comp]} for comp in range(ncomp)])
